            ...
            >
              ...
              >
                 {school.length!==0?school.map((info, index) => {
                  return (
                   info.lat!==null && info.long!==null? <Placemark
                   onClick={()=>{openModalRegion({regionName:info.branchName, regionNameRu:info.branchNameRu, email:info.email, phoneNumber:info.phoneNumber, mailIndex:info.mailIndex, regionPresident})}}
                      key={index}
                      className="placeMark"
                      geometry={
                          [Number(info.lat), Number(info.long)]
