            ...
            >
              ...
              >
                {data.length!==0?data.map((info, index) => {
                  return (
                   ...
                   onClick={()=>{openModalRegion(info.push({type:'region'}))}}
                      key={index}
                      className="placeMark"
                      geometry={
              ...
              >
                 {school.length!==0?school.map((info, index) => {
                  return (
                   info.lat!==null && info.long!==null? <Placemark
                   onClick={()=>{openModalRegion({regionName:info.branchName, regionNameRu:info.branchNameRu, lat:info.lat, long:info.long, email:info.email, type:"school", regionAdress:info.regionAdress, regionAdressRu:info.regionAdressRu, fax:null, phoneNumber:info.phoneNumber, mailIndex:info.mailIndex, regionPresident:info.branchPresident, regionPresidentRu:info.branchPresidentRu})}}
                      key={index}
                      className="placeMark"
                      geometry={
                          [Number(info.lat), Number(info.long)]
                ...
                {/* {
                    data.map((item, index) => */}
                        <div className="map-modal_content" >
                            <h2 style={{fontSize:region.type==="school"?'24px':''}} className="region_name">{t("check")?region.regionName!==null?region.regionName:region.regionNameRu:region.regionNameRu!==null?region.regionNameRu:region.regionName}</h2>
