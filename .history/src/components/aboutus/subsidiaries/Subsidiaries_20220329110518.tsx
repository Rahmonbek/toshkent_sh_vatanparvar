...
import { region, regionI, urlImg, urlPer } from "../../../host";
...
<Link className="butS" to={`/tashkilot/${item.branchId}`}>{t("FILIAL HAQIDA")} <AiOutlineRight style={{position:'relative', top:"3px"}} size="1.2rem"/></Link>
         ...
         <div className="rahbariyat-list sal">
              ...
              <div className="content-wrapper">
               <Image  className="img" src={`${urlImg}/${data!==null??data[0].webSiteProfileImageUrl}`} alt="img" />
               <div className="rahbariyat-list-text sug">
                 <p  className="rahbariyat-list-text-position" style={{marginTop:"10px"}}>
                   <b>
                   {t('check')?data!==null??data[0].branchName:data!==null??data[0].branchNameRu}       
                 </b></p>
   
                 <p  className="rahbariyat-list-text-name">
                 {t('check')?data!==null??data[0].branchPresident:data!==null??data[0].branchPresidentRu}
                 {/* Ismoilov Rahmon Zohidovich */}
                 </p>
                 <div className="icons" style={{marginTop:'10px'}}>
                   <FaRegCalendarCheck  className="icon"  size="1.5rem" color="#244BA4" cursor="pointer"/>
                   <p  style={{position:'relative', top:'-14px'}}>{t('check')?data!==null??data[0].visitingDaysUz:data!==null??data[0].visitingDaysRu}</p>
                 </div>
                 <div className="icons">
                   <FiPhone className="icon"  size="1.5rem" color="#244BA4" cursor="pointer"/>
                   <p  style={{position:'relative', top:'-14px'}}><a href={`tel: ${data!==null??data[0].phoneNumber}`}>{data!==null??data[0].phoneNumber}</a></p>
                 </div>
               
                 <div className="icons">
                   <HiOutlineMail className="icon iconn" size="1.5rem" color="#244BA4" cursor="pointer"/>
                   <p  style={{position:'relative', top:'-14px'}}><a href={`mailto: ${data!==null??data[0].email}`}>{data!==null??data[0].email}</a></p>
                 </div>
                
                 <div className="web" style={{height:'40px', marginTop:'-60px'}}>
 <Link className="butS" to={`/tashkilot/${data!==null??data[0].branchId}`}>{t("FILIAL HAQIDA")} <AiOutlineRight style={{position:'relative', top:"3px"}} size="1.2rem"/></Link>
               </div>
               </div>
               </div> 
             </div>
