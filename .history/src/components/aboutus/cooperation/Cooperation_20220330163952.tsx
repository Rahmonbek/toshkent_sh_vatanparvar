...
import card1 from '../leaders/assets/user.webp'
...
<br/><br/><br/> */}
   ...
   <p className={style.narx}><b>{item.price===null?t("Bepul"):item.price}</b></p>
...
<Carousel
   ...
   <ul>
     {item.sportSections!==null && item.sportSections.length!==0?item.sportSections.map(item1=>{
return( <li>{t("check")?item.nameUz!==null?item.nameUz:item.nameRu:item.nameRu!==null?item.nameRu:item.nameUz}</li>)
     }):''}
   <
   </ul>
   <Link to="/" className={style.but}>Ariza yuborish</Link>
