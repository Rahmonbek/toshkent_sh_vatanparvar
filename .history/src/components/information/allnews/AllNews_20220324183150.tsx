...
if(t('check')){
    if(pageNumber===1 && bosh+6<dataUz.length/6){
        setCurrentPage(currentPage+pageNumber)
    ...
    if(pageNumber===1 && bosh+6<dataRu.length/6){
        setCurrentPage(currentPage+pageNumber)
        setbosh(Number(bosh)+Number(pageNumber))
    ...
    return (
        <div className="all-news tnew">
            ...
            <Row style={{justifyContent:'center', width:'100%', marginRight:'0px'}}>

{
   
   t("check")?
   currentPostsUz.map((item, index) =>

            
            <Col style={{padding:'10px', width:'100%'}} lg={8}  style={{}} md={12} sm={24}>
      ...
      <div className='blackPar'>
        ...
        <div className="tags">
          ...
          <div className="tag"><AiFillRightCircle size="2em" style={{position:'relative',  top:'-10px'}} className="icon"/></div>
                ...
                <Pagination
                        paginate={paginate}
                        totalPosts={t("check")?dataUz.length:dataRu.length}
