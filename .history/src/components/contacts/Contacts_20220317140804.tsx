...
//  }]
  async function getContacts(){
    ...
    .then((res) => {
       var b=[]
       b.push(res.data.filter((data: any) => data.regionId === regin))
      ...
      <PageTitle title={t("KONTAKTLAR")}/>
        ...
        <div className="sal">
                      ...
                      <Panel className={`${0===Number(number)?"Active":''} panel`}  style={{zIndex:-1}} key={0} header={
                              ...
                              <div style={{zIndex:-1}} className={style.headName}> {region.length!==0?t('check')?region[index - 1][0].regionName:region[index - 1][0].regionNameRu:''}</div>
                             </div>}>
                             <Row>
...
<div className={style.op}>
                      ...
                      <Panel className={`${0===Number(number)?"Active":''} panel`}  style={{zIndex:-1}} key={0} header={
                              ...
                              <div style={{zIndex:-1}} className={style.headId}>{1}</div>
                              <div style={{zIndex:-1}} className={style.headName}> {region.length!==0?t('check')?region[index - 1][0].regionName:region[index - 1][0].regionNameRu:''}</div>
                              <div style={{zIndex:-1}} className={style.headPerson}>{region.length!==0?t('check')?region[index - 1][0].regionPresident:region[index - 1][0].regionPresidentRu:''}</div>
                              <div style={{zIndex:-1}} className={style.headPhone}>{region.length!==0?region[index - 1][0].phoneNumber===null?"-":region[index - 1][0].phoneNumber:''}</div>
                              <div style={{zIndex:-1}} className={style.headEmail}>{region.length!==0?region[index - 1][0].email==="null"?"-":region[index - 1][0].email:''}</div>
                            </div>}>
                            <Row>
                            <Col className="colAcc" md={12} sm={24}>
<Tooltip placement="left" title={t("Bank nomi")}><RiBankFill  color="#133165" size='1.4rem' /> <span className="salomText">{region.length!==0?t('check')?(region[index - 1][0].bankName && region[index - 1][0].bankName!==null?region[index - 1][0].bankName:region[index - 1][0].bankNameRu && region[index - 1][0].bankNameRu!==null?region[index - 1][0].bankNameRu:<span style={{color:'#a2a0a0'}}>{t("Ma'lumot mavjud emas")}</span>):region[index - 1][0].bankNameRu && region[index - 1][0].bankNameRu!==null?region[index - 1][0].bankNameRu:region[index - 1][0].bankName && region[index - 1][0].bankName!==null?region[index - 1][0].bankName:<span style={{color:'#a2a0a0'}}>{t("Ma'lumot mavjud emas")}</span>:''}</span></Tooltip>
