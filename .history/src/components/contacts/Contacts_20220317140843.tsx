      ...
      <PageTitle title={t("KONTAKTLAR")}/>
        ...
        <div className="sal">
                      ...
                      <Panel className={`${0===Number(number)?"Active":''} panel`}  style={{zIndex:-1}} key={0} header={
                              ...
                              <div style={{zIndex:-1}} className={style.headName}> {region.length!==0?t('check')?region][0].regionName:region][0].regionNameRu:''}</div>
                             </div>}>
                             <Row>
<Col style={{padding:'10px'}} className="colAcc" md={12} sm={24}>
  <BsPerson size='1.3rem'  color="#133165" /><span className="salomText">{region.length!==0?t('check')?(region][0].regionPresident && region][0].regionPresident!==null?region][0].regionPresident:region][0].regionPresidentRu && region][0].regionPresidentRu!==null?region][0].regionPresidentRu:<span style={{color:'#a2a0a0'}}>{t("Ma'lumot mavjud emas")}</span>):region][0].regionPresidentRu && region][0].regionPresidentRu!==null?region][0].regionPresidentRu:region][0].regionPresident && region][0].regionPresident!==null?region][0].regionPresident:<span style={{color:'#a2a0a0'}}>{t("Ma'lumot mavjud emas")}</span>:''}</span>
  </Col>
  <Col style={{padding:'10px'}} className="colAcc" md={12} sm={24}>
  <AiOutlinePhone size='1.3rem'  color="#133165" /><span className="salomText">{region.length!==0?region][0].phoneNumber===null?<span style={{color:'#a2a0a0'}}>{t("Ma'lumot mavjud emas")}</span>:region][0].phoneNumber:''}</span>
  </Col>
  <Col style={{padding:'10px'}} className="colAcc" md={12} sm={24}>
  <HiOutlineMailOpen size='1.3rem'  color="#133165" /><span className="salomText">{region.length!==0?region][0].email==="null"?<span style={{color:'#a2a0a0'}}>{t("Ma'lumot mavjud emas")}</span>:region][0].email:''}</span>
  </Col>
  <Col className="colAcc" md={12} sm={24}>
<Tooltip placement="left" title={t("Bank nomi")}><RiBankFill  color="#133165" size='1.4rem' /> <span className="salomText">{region.length!==0?t('check')?(region][0].bankName && region][0].bankName!==null?region][0].bankName:region][0].bankNameRu && region][0].bankNameRu!==null?region][0].bankNameRu:<span style={{color:'#a2a0a0'}}>{t("Ma'lumot mavjud emas")}</span>):region][0].bankNameRu && region][0].bankNameRu!==null?region][0].bankNameRu:region][0].bankName && region][0].bankName!==null?region][0].bankName:<span style={{color:'#a2a0a0'}}>{t("Ma'lumot mavjud emas")}</span>:''}</span></Tooltip>
  </Col>
 
  
  <Col className="colAcc" md={12} sm={24}>
  <Tooltip placement="left" title={t("Tashkilot manzili")}><FaMapMarkerAlt  color="#133165" size='1.4rem' />  <span className="salomText">{region.length!==0?t('check')?(region][0].regionAdress && region][0].regionAdress!==null?region][0].regionAdress:region][0].regionAdressRu && region][0].regionAdressRu!==null?region][0].regionAdressRu:<span style={{color:'#a2a0a0'}}>{t("Ma'lumot mavjud emas")}</span>):region][0].regionAdressRu && region][0].regionAdressRu!==null?region][0].regionAdressRu:region][0].regionAdress && region][0].regionAdress!==null?region][0].regionAdress:<span style={{color:'#a2a0a0'}}>{t("Ma'lumot mavjud emas")}</span>:''}</span></Tooltip>
  </Col>
  <Col className="colAcc" md={12} sm={24}>
  <Tooltip placement="left" title={t("Tashkilot hisob raqami")}><RiBankCardFill  color="#133165" size='1.4rem' /> <span className="salomText">{region.length!==0?region.length!==0?region][0].bankAccount===null?<span style={{color:'#a2a0a0'}}>{t("Ma'lumot mavjud emas")}</span>:region][0].bankAccount.slice(0, 4)+" "+region][0].bankAccount.slice(4, 8)+" "+region][0].bankAccount.slice(8, 12)+" "+region][0].bankAccount.slice(12, 16)+" "+region][0].bankAccount.slice(16, 20):'':''}</span></Tooltip>
  </Col>
  <Col className="colAcc" md={12} sm={24}>
  <Tooltip placement="left" title={t("INN")}><b style={{color:'#133165'}}>INN</b> <span style={{top:'0px'}} className="salomText">{region.length!==0?region.length!==0?region][0].inn===null?<span style={{color:'#a2a0a0'}}>{t("Ma'lumot mavjud emas")}</span>:region][0].inn.slice(0, 3)+" "+region][0].inn.slice(3, 6)+" "+region][0].inn.slice(6, 9):'':''}</span></Tooltip>
  </Col>
 
  <Col className="colAcc" md={12} sm={24}>
  <Tooltip placement="left" title={t("MFO")}> <b style={{color:'#133165'}}>MFO</b> <span style={{top:'0px'}} className="salomText">{region.length!==0?region.length!==0?region][0].bankMfo===null?<span style={{color:'#a2a0a0'}}>{t("Ma'lumot mavjud emas")}</span>:region][0].bankMfo:'':''}</span></Tooltip>
  </Col>
  <Col className="colAcc" md={12} sm={24}>
  <Tooltip placement="left" title={t("Pochta indeks")}><BsFillEnvelopeFill color="#133165" size='1.4rem'/> <span className="salomText">{region.length!==0?region.length!==0?region][0].mailIndex===null?<span style={{color:'#a2a0a0'}}>{t("Ma'lumot mavjud emas")}</span>:region][0].mailIndex:'':''}</span></Tooltip>
  </Col>
...
<div className={style.op}>
                      ...
                      <Panel className={`${0===Number(number)?"Active":''} panel`}  style={{zIndex:-1}} key={0} header={
                              ...
                              <div style={{zIndex:-1}} className={style.headId}>{1}</div>
                              <div style={{zIndex:-1}} className={style.headName}> {region.length!==0?t('check')?region][0].regionName:region][0].regionNameRu:''}</div>
                              <div style={{zIndex:-1}} className={style.headPerson}>{region.length!==0?t('check')?region][0].regionPresident:region][0].regionPresidentRu:''}</div>
                              <div style={{zIndex:-1}} className={style.headPhone}>{region.length!==0?region][0].phoneNumber===null?"-":region][0].phoneNumber:''}</div>
                              <div style={{zIndex:-1}} className={style.headEmail}>{region.length!==0?region][0].email==="null"?"-":region][0].email:''}</div>
                            </div>}>
                            <Row>
                            <Col className="colAcc" md={12} sm={24}>
<Tooltip placement="left" title={t("Bank nomi")}><RiBankFill  color="#133165" size='1.4rem' /> <span className="salomText">{region.length!==0?t('check')?(region][0].bankName && region][0].bankName!==null?region][0].bankName:region][0].bankNameRu && region][0].bankNameRu!==null?region][0].bankNameRu:<span style={{color:'#a2a0a0'}}>{t("Ma'lumot mavjud emas")}</span>):region][0].bankNameRu && region][0].bankNameRu!==null?region][0].bankNameRu:region][0].bankName && region][0].bankName!==null?region][0].bankName:<span style={{color:'#a2a0a0'}}>{t("Ma'lumot mavjud emas")}</span>:''}</span></Tooltip>
...
<Col className="colAcc" md={12} sm={24}>
  <Tooltip placement="left" title={t("Tashkilot hisob raqami")}><RiBankCardFill  color="#133165" size='1.4rem' /> <span className="salomText">{region.length!==0?region][0].bankAccount===null?<span style={{color:'#a2a0a0'}}>{t("Ma'lumot mavjud emas")}</span>:region][0].bankAccount.slice(0, 4)+" "+region][0].bankAccount.slice(4, 8)+" "+region][0].bankAccount.slice(8, 12)+" "+region][0].bankAccount.slice(12, 16)+" "+region][0].bankAccount.slice(16, 20):''}</span></Tooltip>
  </Col>
<Col className="colAcc" md={12} sm={24}>
  <Tooltip placement="left" title={t("INN")}><b style={{color:'#133165'}}>INN</b> <span style={{top:'0px'}} className="salomText">{region.length!==0?region][0].inn===null?<span style={{color:'#a2a0a0'}}>{t("Ma'lumot mavjud emas")}</span>:region][0].inn.slice(0, 3)+" "+region][0].inn.slice(3, 6)+" "+region][0].inn.slice(6, 9):''}</span></Tooltip>
  </Col>
<Col className="colAcc" md={12} sm={24}>
  <Tooltip placement="left" title={t("MFO")}> <b style={{color:'#133165'}}>MFO</b> <span style={{top:'0px'}} className="salomText">{region.length!==0?region][0].bankMfo===null?<span style={{color:'#a2a0a0'}}>{t("Ma'lumot mavjud emas")}</span>:region][0].bankMfo:''}</span></Tooltip>
  </Col>
<Col className="colAcc" md={12} sm={24}>
  <Tooltip placement="left" title={t("Pochta indeks")}><BsFillEnvelopeFill color="#133165" size='1.4rem'/> <span className="salomText">{region.length!==0?region][0].mailIndex===null?<span style={{color:'#a2a0a0'}}>{t("Ma'lumot mavjud emas")}</span>:region][0].mailIndex:''}</span></Tooltip>
  </Col>
