            ...
            >
              ...
              >
                 {school.length!==0?school.map((info, index) => {
                  return (
                   info.lat!==null && info.long!==null? <Placemark
                   onClick={()=>{openModalRegion({regionName:info.branchName, regionNameRu:info.branchNameRu, email:info.email, regionAdress:info.regionAdress, regionAdress:info.regionAdressRu, phoneNumber:info.phoneNumber, mailIndex:info.mailIndex, regionPresident:info.branchPresident, regionPresidentRu:info.branchPresidentRu})}}
                      key={index}
                      className="placeMark"
                      geometry={
                          [Number(info.lat), Number(info.long)]
